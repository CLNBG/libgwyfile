/*
 * $Id: item_string_array_owned_owned.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test string array item with owned data.
 *
...
main(void)
{
    ...
    for (i = 0; i < len; i++) {
        assert(strcmp(v[i], constvalue[i]) == 0);
    }
    assert(gwyfile_item_get_string_array(item) == (const char*const*)value);
