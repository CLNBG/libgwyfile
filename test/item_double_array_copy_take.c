/*
 * $Id: item_double_array_copy_take.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test double array item with copied and taken data.
 *
...
main(void)
{
    ...
    takenvalue = gwyfile_item_take_double_array(item);
    ...
    assert(memcmp(gwyfile_item_get_double_array(item), value,
                  len*sizeof(double)) == 0);
    assert(memcmp(gwyfile_item_get_double_array(item), takenvalue,
                  len*sizeof(double)) == 0);
    assert(gwyfile_item_get_double_array(item) == takenvalue);
    gwyfile_item_free(item);
    assert(memcmp(value, takenvalue, len*sizeof(double)) == 0);
    free(takenvalue);
