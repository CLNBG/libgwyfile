/*
 * $Id: membuf_object_write_too_short.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test error handling when writing into truncated memory buffers.
 *
...
main(void)
{
    ...
    for (i = 0; i < size; i++) {
        bytes_written = gwyfile_object_write_memory(object, buffer, i, &error);
