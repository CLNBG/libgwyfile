/*
 * $Id: membuf_item_double_array_read_too_short.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    double values[] = { 1.25, M_PI, -1e-6 };
    GwyfileItem *item;
    GwyfileError *error = NULL;
    size_t i, size, bytes_written, bytes_read;
    char *buffer;

    item = gwyfile_item_new_double_array_const("array", values, 3);
    assert(item);
