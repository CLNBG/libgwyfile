/*
 * $Id: object_selectionlattice_copy.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    ...
    assert(gwyfile_object_selectionprojective_get(selection, NULL, "nsel", &nsel, NULL));
    assert(nsel == n);

    assert(gwyfile_object_selectionprojective_get(selection, NULL, "data", &data3, NULL));
    assert(memcmp(data3, data, n*itemsize*sizeof(double)) == 0);

    assert(gwyfile_object_selectionprojective_get(selection, NULL, "data(take)", &data2, NULL));
