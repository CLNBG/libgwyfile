/*
 * $Id: membuf_item_int64_array.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    const int64_t value[] = { -11, 0, INT64_MAX };
    ...
    item = gwyfile_item_new_int64_array_const("int64s", value, len);
    assert(item);

    size = gwyfile_item_size(item);
    assert(size == strlen("int64s")+1 + 1 + sizeof(uint32_t) + sizeof(value));
    ...
    for (i = 0; i < len; i++) {
        assert(gwyfile_item_get_int64_array(item)[i] == value[i]);
    }
    assert(strcmp(gwyfile_item_name(item), "int64s") == 0);
