/*
 * $Id: file_error_itemtype.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test item type error.
 *
...
main(void)
{
    ...
    sprintf(filename, "file_error_itemtype.%u.gwy", getpid());
    ...
    assert(fh);
    assert(fseek(fh, 15L, SEEK_SET) == 0);
    assert(fwrite(";", 1, 1, fh) == 1);
    fclose(fh);
    ...
    assert(error->code == GWYFILE_ERROR_ITEM_TYPE);
