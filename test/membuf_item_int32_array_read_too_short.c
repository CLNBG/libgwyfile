/*
 * $Id: membuf_item_int32_array_read_too_short.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    int32_t values[] = { 1, 2, 15 };
    GwyfileItem *item;
    GwyfileError *error = NULL;
    size_t i, size, bytes_written, bytes_read;
    char *buffer;

    item = gwyfile_item_new_int32_array_const("array", values, 3);
    assert(item);
