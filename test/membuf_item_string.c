/*
 * $Id: membuf_item_string.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    ...
    item = gwyfile_item_new_string_const("string", "Shark");
    assert(item);

    size = gwyfile_item_size(item);
    assert(size == strlen("string")+1 + 1 + strlen("Shark")+1);
    ...
    assert(strcmp(gwyfile_item_name(item), "string") == 0);
