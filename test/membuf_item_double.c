/*
 * $Id: membuf_item_double.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    ...
    item = gwyfile_item_new_double("double", M_PI);
    assert(item);

    size = gwyfile_item_size(item);
    assert(size == strlen("double")+1 + 1 + sizeof(double));
    ...
    assert(strcmp(gwyfile_item_name(item), "double") == 0);
