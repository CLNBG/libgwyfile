/*
 * $Id: object_graphcurvemodel_owned.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test GwyGraphCurveModel object constructor and extractor with owned data.
 *
...
main(void)
{
    ...
    double *xdataowned, *ydataowned, *xdata2, *ydata2;
    ...
    xdataowned = malloc(3*sizeof(double));
    memcpy(xdataowned, xdata, 3*sizeof(double));
    ydataowned = malloc(3*sizeof(double));
    memcpy(ydataowned, ydata, 3*sizeof(double));
    ...
    assert(strcmp(gwyfile_object_name(graphcurvemodel), "GwyGraphCurveModel")
           == 0);
    gwyfile_object_add(graphcurvemodel, gwyfile_item_new_int32("EXTRA_ITEM",
                                                               -10));
    ...
    assert(gwyfile_object_graphcurvemodel_get(graphcurvemodel, NULL,
                                              ...
                                              "ydata(take)", &ydata2,
                                              NULL));
    assert(memcmp(xdata2, xdata, 3*sizeof(double)) == 0);
    assert(memcmp(ydata2, ydata, 3*sizeof(double)) == 0);
