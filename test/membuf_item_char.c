/*
 * $Id: membuf_item_char.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
...
main(void)
{
    ...
    item = gwyfile_item_new_char("char", 'G');
    assert(item);

    size = gwyfile_item_size(item);
    assert(size == strlen("char")+1 + 1 + 1);
    ...
    assert(gwyfile_item_get_char(item) == 'G');
    assert(strcmp(gwyfile_item_name(item), "char") == 0);
