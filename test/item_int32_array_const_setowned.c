/*
 * $Id: item_int32_array_const_setowned.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test int32 array item with constant and owned data.
 *
...
main(void)
{
    ...
    const int32_t newconstvalue[] = { 4, 5, 7, INT32_MIN };
    int32_t *newvalue = memcpy(malloc(newlen*sizeof(int32_t)), newconstvalue,
                               newlen*sizeof(int32_t));

    item = gwyfile_item_new_int32_array_const("item", value, len);
    assert(item);
    assert(gwyfile_item_type(item) == GWYFILE_ITEM_INT32_ARRAY);
    gwyfile_item_set_int32_array(item, newvalue, newlen);
