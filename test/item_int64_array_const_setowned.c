/*
 * $Id: item_int64_array_const_setowned.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Test int64 array item with constant and owned data.
 *
...
main(void)
{
    ...
    const int64_t newconstvalue[] = { 4, 5, 7, INT64_MIN };
    int64_t *newvalue = memcpy(malloc(newlen*sizeof(int64_t)), newconstvalue,
                               newlen*sizeof(int64_t));

    item = gwyfile_item_new_int64_array_const("item", value, len);
    assert(item);
    assert(gwyfile_item_type(item) == GWYFILE_ITEM_INT64_ARRAY);
    gwyfile_item_set_int64_array(item, newvalue, newlen);
