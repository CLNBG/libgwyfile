/*
 * $Id: check_utf8_name.c 363 2022-02-10 12:15:04Z yeti-dn $
 *
 * Invalid UTF-8 component name validity error.
 *
...
main(void)
{
    ...
    object = gwyfile_object_new
                 ("TopLevel",
                  ...
                  gwyfile_item_new_int32("i\xff", 42),
                  gwyfile_item_new_object
                      ("obj",
                       gwyfile_object_new
                           ("Nested",
                            gwyfile_item_new_int32("invalid\xfe", 41),
                            NULL)),
                  NULL);
    assert(object);
    ...
    for (i = 0; i < errlist.n; i++) {
        ...
        assert(error->code == GWYFILE_INVALID_UTF8_NAME);
    }
